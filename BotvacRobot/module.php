...
class BotvacRobot extends IPSModule
{
    ...
    public function Update()
    {
		...
		$map_boundaries_id = $this->RegisterVariableString('MAPBOUNDARIES', 'Map Boundaries', '', 3);
    ...
    private function UpdateCommandProfile($availables = false)
    {
        ...
        if (IPS_VariableProfileExists($name) && $availables !== false) {
        ...
        if ($availables !== false) {
            if ($availables['start'] == 1) {
                IPS_SetVariableProfileAssociation($name, 1, 'Reinigung: Haus', '', -1);
            }
            if ($availables['start'] == 1) {
                IPS_SetVariableProfileAssociation($name, 2, 'Reinigung: Spot', '', -1);
            }
            if ($availables['stop'] == 1) {
                IPS_SetVariableProfileAssociation($name, 6, 'Stop', '', -1);
            }
            if ($availables['pause'] == 1) {
                IPS_SetVariableProfileAssociation($name, 7, 'Pause', '', -1);
            }
            if ($availables['resume'] == 1) {
                IPS_SetVariableProfileAssociation($name, 8, 'Wiederaufnahme', '', -1);
            }
            if ($availables['goToBase'] == 1) {
                IPS_SetVariableProfileAssociation($name, 9, 'Zur Station', '', -1);
            }
        }
    }
    ...
    private function UpdateZonesProfile($zones_array = false)
    {
		...
		$i = 0;
        foreach ($zones_array['data']['boundaries'] as $zone) {
			$i++;
			if ($zone['name'] <> "")
			{
